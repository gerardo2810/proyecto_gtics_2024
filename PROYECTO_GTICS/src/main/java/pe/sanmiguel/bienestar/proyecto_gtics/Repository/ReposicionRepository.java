...
public interface ReposicionRepository extends JpaRepository<Reposicion, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT r.*, CONCAT(s.id) AS 'SedeID', s.idAdmin, s.direccion, s.nombre, CONCAT(f.id) AS 'idFarmacista', f.idRol, f.correo, f.contrasena, f.nombres, f.apellidos, f.celular, f.dni, f.direccion\n" +
            "FROM proyecto_gtics.reposicion r, proyecto_gtics.sede s, proyecto_gtics.usuario f WHERE r.idEstado = s.id and s.idAdmin = f.id and s.idAdmin IS NOT NULL;")
